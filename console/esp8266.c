...
void ESP8266_InitUART(uint32_t baud, int echo){ volatile int delay;
  ...
  //UART1_IBRD_R = 8000000/baud;   
  //UART1_FBRD_R = ((64*(8000000%baud))+baud/2)/baud;      
...
#define MESS_START "+IPD,"

static char SearchBuffer[6];				// buffer for finding message start string
static uint8_t Searching = TRUE;							// toggles transmitted message start search 
static uint8_t MessageStart = FALSE;					// toggles sent message start

static char LengthBuffer[5];
static uint8_t LengthBufferIndex;
static uint16_t MessageLength;

static char LatestMessage[BUFFER_SIZE];
static uint16_t LatestMessageLength;
//----------ESP8266FIFOtoBuffer----------
    ...
    if(ESP8266_EchoResponse){
      //UART_OutCharNonBlock(letter); // echo
		...
		if(Searching){
			...
			SearchBuffer[1] = SearchBuffer[2];
			SearchBuffer[2] = SearchBuffer[3];
			SearchBuffer[3] = SearchBuffer[4];
			SearchBuffer[4] = letter;
			SearchBuffer[5] = '\0';
			if(strcmp(SearchBuffer, MESS_START) == 0){
				LengthBufferIndex = 0;
				MessageLength = 0;
				Searching = FALSE;
				memset(RXBuffer, 0, BUFFER_SIZE * sizeof(char));
				memset(LengthBuffer, 0, 4 * sizeof(char));
				memset(SearchBuffer, 0, 6 * sizeof(char));
			}
		}// end shifting
		else{
			if((RXBufferIndex >= MessageLength - 1 && MessageStart)|| letter == '\0' || letter == '\n' || letter == '\r' || RXBufferIndex >= BUFFER_SIZE - 1){
				RXBuffer[RXBufferIndex] = letter;
				RXBuffer[RXBufferIndex + 1] = '\0';
				RXBufferIndex = 0; 
				Searching = TRUE;
				MessageStart = FALSE;
				LatestMessageLength = MessageLength;
				memset(LatestMessage, 0, BUFFER_SIZE * sizeof(char));
				memcpy(LatestMessage, RXBuffer, MessageLength * sizeof(char));
				Set_Latest_Packet(LatestMessage);
			}
			else{
				if(MessageStart){
					RXBuffer[RXBufferIndex] = letter;
					...
					MessageStart = TRUE;
					MessageLength = atoi(LengthBuffer);
					RXBufferIndex = 0;
				}
				else{
					LengthBuffer[LengthBufferIndex] = letter;
					LengthBufferIndex ++;
				}
			}
		}
...
void Get_LatestMessage(char* buffer){
	strcpy(buffer, LatestMessage);
}
uint16_t Get_LatestMessageLength(void){
	return LatestMessageLength;
}
...
//  }
	...
	switch(mode){
		case SOFTAP:
			// step 2: AT+CWMODE=2 set wifi mode to access point
			if(ESP8266_SetWifiMode(ESP8266_WIFI_MODE_AP)==0){ 
				printf("SetWifiMode, could not set mode\n\r"); while(1){};
			}		
			if(ESP8266_ConfigureAccessPoint(SSID_NAME, PASSKEY, 5, 4) == 0){
				printf("ConfigureAccessPoint, could not configure\n\r"); while(1){};
			}
			...
			//// optional step: AT+CIPMUX==1 set mode to multi socket
			if(ESP8266_SetConnectionMux(0)==0){
				printf("SetConnectionMux error, could not set connection mux\n\r"); while(1){};
			}
			if(ESP8266_MakeUDPConnection(SERVER_IP, UDP_PORT, UDP_PORT) == 0){
				printf("MakeUDPConnection, could not make connection\n\r"); while(1){};
			}
			break;
		case CLIENT:
			...
			if(ESP8266_MakeUDPConnection(SERVER_IP, SERVER_PORT, UDP_PORT) == 0){
				printf("MakeUDPConnection, could not make connection\n\r"); while(1){};
			}
			break;
		default:
	...
	ESP8266_InputProcessingEnabled = true; // not a server; might need to change
...
int ESP8266_MakeUDPConnection(char *IPaddress, uint32_t serverPort, uint32_t UDPport){
  ...
  SearchStart("ok");
  while(try){
    sprintf((char*)TXBuffer, "AT+CIPSTART=\"UDP\",\"%s\",%d,%d,0\r\n", IPaddress, serverPort, UDPport);
...
int ESP8266_SetDHCP(uint8_t mode, uint8_t en) {
  ...
  SearchStart("ok");
  while(try){
    sprintf((char*)TXBuffer, "AT+CWDHCP_DEF=%d,%d\r\n", mode, en);
...
int ESP8266_SetStationIP(char *IPaddress){
  ...
  SearchStart("ok");
  while(try){
    sprintf((char*)TXBuffer, "AT+CIPSTART=\"%s\"", IPaddress);
    ESP8266SendCommand(TXBuffer);   // open and connect to a socket
    DelayMsSearching(8000);
    if(SearchFound) return 1; // success
    try--;
...
int ESP8266_SetBaud(int baud) {
  ...
  SearchStart("ok");
  while(try){
    sprintf((char*)TXBuffer, "AT+UART_CUR=%d,8,1,0,0\r\n", baud);
    ESP8266SendCommand((const char*)TXBuffer);
    DelayMsSearching(5000);
    if(SearchFound) return 1; // success
    try--;
...
int ESP8266_SetWifiMode(uint8_t mode){
  ...
  SearchStart("ok");
  while(try){
    sprintf((char*)TXBuffer, "AT+CWMODE_DEF=%d\r\n", mode);
    ESP8266SendCommand((const char*)TXBuffer);
    DelayMsSearching(5000);
    if(SearchFound) return 1; // success
    try--;
...
int ESP8266_ConfigureAccessPoint(const char* ssid, const char* password, uint8_t channel, uint8_t encryptMode){
  ...
  SearchStart("ok");
  while(try){
    sprintf((char*)TXBuffer, "AT+CWSAP_DEF=\"%s\",\"%s\",%d,%d\r\n", ssid, password, channel, encryptMode);
    ESP8266SendCommand((const char*)TXBuffer);
    DelayMsSearching(4000);
    if(SearchFound) return 1; // success
    try--;
